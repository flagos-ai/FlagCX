...
TEST(ProxyConnectionState, FailedConnectionRejectsNewControlWork) {
  flagcxProxyState state{};
  ...
  flagcxProxyRecordConnectionError(&connection, flagcxRemoteError);
  EXPECT_EQ(flagcxProxyCallAsync(&comm, &connector, flagcxProxyMsgConnect,
                                 nullptr, 0, 0, &connection),
            flagcxRemoteError);
}

TEST(ProxyControlRpc, BlockingCallReturnsTerminalRemoteError) {
  int fds[2] = {-1, -1};
  ...
  std::thread server([&]() {
    ...
    serverOk = type == flagcxProxyMsgConnect && requestSize == 0 &&
