...
TEST_F(DeviceAdaptorTest, DeviceMemoryAlloc) {
  ...
  auto result = devHandle->deviceMalloc(&devPtr, TEST_SIZE, flagcxMemDevice, stream);
  EXPECT_EQ(result, flagcxSuccess);
...
TEST_F(DeviceAdaptorTest, HostMemoryAlloc) {
  ...
  auto result = devHandle->deviceMalloc(&hostPtr, TEST_SIZE, flagcxMemHost, stream);
  EXPECT_EQ(result, flagcxSuccess);
...
TEST_F(DeviceAdaptorTest, MemoryCopy) {
  ...
  // Allocate memory
  ASSERT_EQ(devHandle->deviceMalloc(&hostSrc, TEST_SIZE, flagcxMemHost, stream), flagcxSuccess);
  ASSERT_EQ(devHandle->deviceMalloc(&hostDst, TEST_SIZE, flagcxMemHost, stream), flagcxSuccess);
  ASSERT_EQ(devHandle->deviceMalloc(&devPtr, TEST_SIZE, flagcxMemDevice, stream), flagcxSuccess);
  ...
  // Copy: Host -> Device
  auto result = devHandle->deviceMemcpy(devPtr, hostSrc, TEST_SIZE, flagcxMemcpyHostToDevice, stream);
  EXPECT_EQ(result, flagcxSuccess);
  
  // Copy: Device -> Host
  result = devHandle->deviceMemcpy(hostDst, devPtr, TEST_SIZE, flagcxMemcpyDeviceToHost, stream);
  EXPECT_EQ(result, flagcxSuccess);
  ...
  for (size_t i = 0; i < TEST_COUNT; i++) {
    EXPECT_FLOAT_EQ(dstData[i], static_cast<float>(i)) << "Mismatch at index " << i;
  }
...
TEST_F(DeviceAdaptorTest, MemorySet) {
  ...
  // Allocate memory
  ASSERT_EQ(devHandle->deviceMalloc(&hostPtr, TEST_SIZE, flagcxMemHost, stream), flagcxSuccess);
  ASSERT_EQ(devHandle->deviceMalloc(&devPtr, TEST_SIZE, flagcxMemDevice, stream), flagcxSuccess);
  
  // Set device memory to 0
  auto result = devHandle->deviceMemset(devPtr, 0, TEST_SIZE, flagcxMemDevice, stream);
  EXPECT_EQ(result, flagcxSuccess);
  
  // Copy back and verify
  result = devHandle->deviceMemcpy(hostPtr, devPtr, TEST_SIZE, flagcxMemcpyDeviceToHost, stream);
  EXPECT_EQ(result, flagcxSuccess);
...
TEST_F(DeviceAdaptorTest, LargeMemoryAlloc) {
  ...
  auto result = devHandle->deviceMalloc(&devPtr, largeSize, flagcxMemDevice, stream);
  EXPECT_EQ(result, flagcxSuccess);
  EXPECT_NE(devPtr, nullptr);
  
  if (devPtr) {
    // Set memory to verify it's accessible
    result = devHandle->deviceMemset(devPtr, 0, largeSize, flagcxMemDevice, stream);
    EXPECT_EQ(result, flagcxSuccess);
