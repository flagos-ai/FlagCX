...
// Test: Get device PCI bus ID
TEST_F(DeviceAdaptorTest, GetDevicePciBusId) {
  if (!devHandle->getDevicePciBusId) {
    GTEST_SKIP() << "getDevicePciBusId not implemented for this backend";
  }
  char busId[64] = {0};
  auto result = devHandle->getDevicePciBusId(busId, sizeof(busId), 0);
  EXPECT_EQ(result, flagcxSuccess);
  EXPECT_GT(strlen(busId), 0u) << "Bus ID string should be non-empty";
  std::cout << "PCI Bus ID: " << busId << std::endl;
}

// Test: Get device properties
TEST_F(DeviceAdaptorTest, GetDeviceProperties) {
  if (!devHandle->getDeviceProperties) {
    GTEST_SKIP() << "getDeviceProperties not implemented for this backend";
  }
  struct flagcxDevProps props;
  memset(&props, 0, sizeof(props));
  auto result = devHandle->getDeviceProperties(&props, 0);
  EXPECT_EQ(result, flagcxSuccess);
  // pciBusId should be populated (non-zero for valid device)
  std::cout << "Device name: " << props.name << std::endl;
  std::cout << "PCI Bus ID (int): " << props.pciBusId << std::endl;
}

// Test: Event elapsed time
TEST_F(DeviceAdaptorTest, EventElapsedTime) {
  if (!devHandle->eventElapsedTime) {
    GTEST_SKIP() << "eventElapsedTime not implemented for this backend";
  }
  ...
  float ms = -1.0f;
  auto result = devHandle->eventElapsedTime(&ms, startEvent, endEvent);
  EXPECT_EQ(result, flagcxSuccess);
