...
#define NCCL_SHRINK_ABORT                                                      \
  0x01 /* First, terminate ongoing parent operations, and then shrink the      \
          parent communicator */
...
/* NCCL_SIM_INFO_INITIALIZER must be assigned to initialize simInfo structure
 * when it is created. Not initialized simInfo will result in NCCL error. */
...
ncclResult_t ncclSend(const void *sendbuff, size_t count,
                      ...
                      cudaStream_t stream);
ncclResult_t pncclSend(const void *sendbuff, size_t count,
                       ncclDataType_t datatype, int peer, ncclComm_t comm,
                       cudaStream_t stream);
